...
import { deleteShipping, fetchShippingById } from "@/service/manage-shipping.service";
...
export function RowActions({ row }: RowActionsProps) {
  ...
  const handleRecordDelete = async (uuid: string) => {
    try {
      const response: any = await deleteShipping(uuid);
  ...
  const handleOpenModal = async (uuid: string) => {
    try {
      const response: any = await fetchShippingById(uuid);
      if (response?.status === true && response?.statusCode === 200) {
        await openPopup('shipping', `${t('Edit')} ${t('Shipping')}`, 'edit', response.data);
      }
    } catch (error: any) {
      toast.error(error?.message);
    }
